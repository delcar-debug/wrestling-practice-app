...
(()=>{
 ...
 function panel(id,coach=false){const sec=document.createElement('section');sec.id=id;sec.className='practice-inbox-panel'+(coach?' coach-inbox-panel':'');const today=new Date().toISOString().slice(0,10);sec.innerHTML=`<div class="practice-inbox-head"><h2>Practice Inbox</h2><span class="practice-inbox-count"></span></div><div class="practice-inbox-form"><input class="inbox-work" type="text" placeholder="What should we work on?"><input class="inbox-by" type="text" placeholder="Added by"><label class="inbox-date-field"><span>Date added</span><input class="inbox-date" type="date" value="${today}"></label><button class="primary inbox-add" type="button">Add</button></div><div class="practice-inbox-list"></div>${coach?'':'<button class="secondary inbox-open-full" type="button" style="margin-top:8px;width:100%">Open full inbox</button>'}`;return sec}
 function ensure(){
